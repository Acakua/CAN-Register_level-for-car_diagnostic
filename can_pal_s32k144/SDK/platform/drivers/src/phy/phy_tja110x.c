...
static status_t sleepAbortReject(uint8_t phy)
{
    return PHY_RMW(phy, EXTENDED_CTRL_ADDR, EXTENDED_CTRL_POWER_MODE_STANDBY, EXTENDED_CTRL_POWER_MODE_MASK);
}
...
static status_t writeExtendedConfig(uint8_t phy, bool config1, bool commonConfig)
{
    ...
    if ((commonConfig == true) && (status == STATUS_SUCCESS))
    {
...
static status_t handleInterruptPwon(uint8_t phy)
{
    ...
    if ((g_phyConfig[phy].extendedConfig != NULL) && (id.typeNo != TJA1100_TYPE_NO))
    {
        status = writeExtendedConfig(phy, true, true); /*TJA1101 or TJA1102_P0 */
        if (status != STATUS_SUCCESS)
        {
            return status;
        }
    }
    if ((id.typeNo == TJA1102P0_TYPE_NO) && (g_phyConfig[phy + 1U].extendedConfig != NULL))
    {
        /* also write specific config to TJA1102_P1 */
        status = writeExtendedConfig((uint8_t) (phy + 1U), true, false);
        if (status != STATUS_SUCCESS)
        {
            return status;
        }
