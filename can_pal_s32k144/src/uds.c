 ...
 */
static void UDS_SendResponse(void) {
    ...
    // If flow is UDS_FLOW_NONE, do nothing.
}
...
static void sf_reportDTCSnapshotByDTCNumber(const CAN_Message_t *requestMsg) {
	...
	if (requestMsg->data[0] != 6) {
		udsCtx.flow = UDS_FLOW_NEG;
		udsCtx.nrc = NRC_INCORRECT_MESSAGE_LENGTH_OR_FORMAT;
		return;
	}
...
void UDS_ReadDTCInformation(const CAN_Message_t *requestMsg) {
	...
	udsCtx.sid = SID_READ_DTC_INFORMATION; // Store SID in context
	udsCtx.nrc = 0;
	udsCtx.payload = NULL;
    ...
    default:
		...
		udsCtx.flow = UDS_FLOW_NEG;
		udsCtx.nrc = NRC_SUB_FUNCTION_NOT_SUPPORTED;
        break;
    }
    UDS_SendResponse();
}
